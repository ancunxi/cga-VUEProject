<template>
  <div class="layout-content">
        ...
        <Col span="4" class="view">
          <a href="javascript:;" class="uploadArea" :title="item.fileName + item.fileType">
			...
			<div class="uploadHoverBtn">
              ...
              <Button v-if="isShowDeleteButton=='true'" type="error" size="small" @click="delConfirm(item.id,item.fileId)">删除</Button>
            </div>            
          </a> 
    ...
    <Row class="center">
      <Col span="24">
        <router-link :to="{path:'../index', query:{moduleId:this.$route.query.moduleId, caseId:this.$route.query.caseId,projectId:$route.query.projectId, cardNumber:$route.query.cardNumber,isShowDeleteButton:this.$route.query.isShowDeleteButton}}"><Button>返回</Button></router-link>
      </Col>
    </Row>
