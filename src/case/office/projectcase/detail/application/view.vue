<template>
  <div class="layout-content">
        ...
        <Col span="4" class="view">
          <a href="javascript:;" class="uploadArea" :title="item.fileName + item.fileType">
            ...
            <div class="uploadHoverBtn">
              <Button type="success" size="small" @click="previews(item.fileId,item.fileType)" style="margin-top:28px;">预览</Button>
    ...
    <Row class="center">
      <Col span="24">
        <router-link :to="{path:'../index', query:{materialId:$route.query.materialId, id:$route.query.id, caseId:$route.query.caseId, projectId:$route.query.projectId, moduleId:$route.query.moduleId,cardNumber:$route.query.cardNumber,applySourceName:$route.query.applySourceName,tabsName:$route.query.tabsName}}"><Button>返回</Button></router-link>         
      </Col>
    </Row>
...
export default{
  ...
  methods: {
    ...
    previews(id,type){
      this.viewFileId = id;
      if(type != '.png' && type != '.jpg' && type != '.bmp' && type != '.jpg'){
