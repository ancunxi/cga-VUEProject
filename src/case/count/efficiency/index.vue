	...
	export default {
					...
					this.endDate = datautil.formatDate(date2, 'yyyy-MM-dd');
				     ...
				     axios.post("/cgaSysLoginInfo/findInfoById.do")
				      .then(data => {
				      	  ...
				      	  var reportURL = this.cjkEncode(this.FineReportUrl + "/ReportServer?reportlet=/cga/caseEfficiceStatistics.cpt&__pi__=false&__bypagesize__=false" +
       ...
       // location.reload();
			...
			exportPdf() {
				...
				var fileName = encodeURI("办事效率统计");
				var reportURL = this.cjkEncode(this.FineReportUrl+"/ReportServer?reportlet=/cga/caseEfficiceStatistics.cpt&__pi__=false&__bypagesize__=false&format=pdf&__filename__=" +fileName+"&startDate=" + this.startDate + "&endDate=" + this.endDate + "&DEPT_ID=" + this.allIdStr);
