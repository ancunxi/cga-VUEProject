...
<script>
	...
	export default {
      ...
      .then(data => {
      	  ...
      	  var reportURL = this.FineReportUrl + "/ReportServer?reportlet=/cga/caseChargerStatistics.cpt&__pi__=false&__bypagesize__=false" +
			...
			exportPdf() {
				...
				var fileName = encodeURI("收费统计");
				var reportURL = this.cjkEncode(this.FineReportUrl + "/ReportServer?reportlet=/cga/caseChargerStatistics.cpt&__pi__=false&__bypagesize__=false&format=pdf&__filename__="+ fileName + "&startDate=" + this.startDate + "&endDate=" + this.endDate + "&deptIds=" + this.allIdStr);
