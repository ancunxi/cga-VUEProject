...
export default {
	...
	methods: {
      ...
      .then(data => {
      	  ...
      	  var reportURL = this.cjkEncode(this.FineReportUrl + "/ReportServer?reportlet=/cga/caseNumStatistics.cpt&__pi__=false&__bypagesize__=false" +
		...
		exportPdf() {
			...
			var fileName = encodeURI("办件量统计");
			var reportURL = this.cjkEncode(this.FineReportUrl+ "/ReportServer?reportlet=/cga/caseNumStatistics.cpt&__pi__=false&__bypagesize__=false&format=pdf&__filename__=" +fileName+
				"&startDate=" + this.startDate + "&endDate=" + this.endDate + "&deptIds=" + this.allIdStr);
