<template>
  ...
  <div>
    <Row>
      <template v-for="item in materialData">
        <Col span="4" class="example">
          <a href="javascript:;" :title="item.exampleName">
    ...
    <!--start  预览-->
    <ul id="dowebok29" style="display: none;">
...
export default{
  ...
  methods:{
    exampleList(materialId){
      ...
      axios.post("/cgaProject/queryProjectExampleList.do", {materialId:materialId})
      .then(data=>{
    ...
    previews(id,name){
      ...
      if(/^.+\.png$/.test(name) == false && /^.+\.jpg$/.test(name) == false && /^.+\.bmp$/.test(name) == false){
        ...
        var viewer = new Viewer(document.getElementById('dowebok29'), {
          url: 'data-original'
        });
        setTimeout(()=>{
          $('#dowebok29 img').click();
