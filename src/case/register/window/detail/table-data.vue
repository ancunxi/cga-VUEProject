  ...
  <Row>
    <Col span="24">
              ...
              <tbody class="ivu-table-tbody">
                <template v-for="item in TableList">
                  <tr class="ivu-table-row">
                    ...
                    <td><div class="ivu-table-cell ellipsis"><span :title="item.materialName" @click="fileNamed(item)" class="layout">{{item.materialName}}</span></div></td>
                      ...
                      <div class="ivu-table-cell">
                        ...
                        <Row class="uploadLocalFile">
                          <Col span="24">
                            ...
                            <form :id="item.id" class="uploadForm">
                              <input type="hidden" name="folder" :value="'cga/' + $route.query.caseId">
                              <input type="hidden" name="source" value="1">
                              <input type="file" name="datafile" class="file" @change="uploadMaterial(item.id, item.caseId, item.materialId)" style="width:43px;height:24px;" accept=".png,.jpg,.jpeg,.gif,.bmp,.pdf,.doc,.docx,.txt,.xls,.xlsx">
                            </form>
                          </Col>
                        </Row>
                    ...
                    <Col span="24">
                      <Form>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="资料编码：">
                              {{projectMatetrials.proMaterialsNo}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="资料名称：">
                              {{projectMatetrials.materialName}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="资料业务编码：">
                              {{projectMatetrials.materialShowNo}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="资料类别：">
                              {{projectMatetrials.materialTypeName}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="提交格式：">
                              {{projectMatetrials.submitFormatName}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="提交途径：">
                              {{projectMatetrials.submitPathwayName}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="份数：">
                              {{projectMatetrials.printNum}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="引用标准资料：">
                              {{projectMatetrials.standardNo}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="是否需要原件：">
                              {{projectMatetrials.isOriginalName}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="原件要求：">
                              {{projectMatetrials.viewOriginal}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="是否申报材料：">
                              {{projectMatetrials.isReportName}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="是否申报必选：">
                              {{projectMatetrials.isDeclareName}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="是否办结后提交：">
                              {{projectMatetrials.isFinishSubmitName}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="办结后提交延时天数：" v-show="projectMatetrials.isFinishSubmit == 0">
                              {{projectMatetrials.delayNum}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col :lg="12" :xs="24">
                            <FormItem label="辅助填表报表文件：">
                              {{projectMatetrials.customformRaqname}}
                            </FormItem>
                          </Col>
                          <Col :lg="12" :xs="24">
                            <FormItem label="在线填表报表文件：">
                              {{projectMatetrials.onlineReportFile}}
                            </FormItem>
                          </Col>
                        </Row>
                        <Row>
                          <Col span="24">
                            <FormItem label="资料条件及要求：">
                              {{projectMatetrials.description}}
                            </FormItem>
                          </Col>
                        </Row>
                      </Form>
      ...
      <Modal
        ...
        <view-modal
          ref='viewChild'
          @parentgetList='getMaterialList'
          :userCardNumber="userCardNumber"
          :userRelatedPersonName="userRelatedPersonName">
        </view-modal>
      </Modal>
...
export default{
  props: ["projectApplyType", "userCardNumber","userRelatedPersonName"],
  components: {
...
//  },
    ...
    addModalFun(caseId,materialId,userCardNumber){
      if(this.userCardNumber !== ''){
        this.addModal = true;
        this.$refs.addChild.getAddList(materialId,userCardNumber);
      }else{
        //根据申请类别给予不同的提示
    ...
    fileNamed(item){
      this.dataModal = true;
      this.projectMatetrials = item;
      this.projectMatetrials.proMaterialsNo = item.proDocId;
    }
  }
}
</script>
