      ...
      <Row class="margin-bottom-20">
        <Col span="24">
          ...
          <Row>
            ...
            <Col span="6">
              <FormItem label="短信通知后台：">
                <RadioGroup v-model="projectData.is_sms_oper">
          ...
          <Row>
            <Col span="12">
              <FormItem
                label="业务编号："
                prop = "code1"
                :required="showCodeRequired"
                >
                <Input v-model="projectData.code1"  class="businessCode" placeholder="请输入业务编号" @on-change="checkedCode(0)" @on-focus="switchCode('code1')" @on-blur="checkedCode1"></Input>
                <div v-if="showCode1Tip" style="color:red;">业务编号和票号任填一个</div>
              </FormItem>
            </Col>
            <Col span="12">
              <FormItem
                ...
                prop = "code2"
                :required="!showCodeRequired">
                <Input v-model="projectData.code2" placeholder="请输入票号" @on-blur="checkedCode(1)" @on-focus="switchCode('code2')"></Input>
                <div v-if="showCode2Tip" style="color:red;">业务编号和票号任填一个</div>
              </FormItem>
            </Col>
          </Row>
        </Col>
          ...
          <Row>
            ...
            <Col span="15">
              <FormItem
                ...
                <Input v-model="userData.cardNumber" placeholder="请输入证件号码" @on-change="checkedIdCard"></Input>
          ...
          <Row>
            ...
            <Col span="7">
              <FormItem
                label="统一社会信用代码："
                prop="cardNumber"
                :rules="[{required: false, max: 30, message: '不能为空且长度不超过30位', trigger: 'blur'}]">
      ...
      <Col span="24">
        ...
        <template>
          <table-data
            ...
            :userRelatedPersonName="userData.relatedPersonName">
          </table-data>
        </template>
      </Col>
    </Row>
...
export default {
  ...
  data () {
    return {
      ...
      showCode1Tip:false,        //业务编码的必填提示
      showCode2Tip:false,        //票号的必填提示
      showCodeRequired:true,      //业务编号和票号任选一个
      cardTypeData: [],        //证件类型
      ...
      relatedModal: false,     //添加关联人信息modal框
      nextAudit: false,        //下一环节审核人是否显示
      mapModal: false,         //百度地图
      //正则表达式
  ...
  watch:{
    serviceType(){
      if(this.serviceType == '02'){
        this.projectData.applyObjectType = '0';
      }else{
        this.projectData.applyObjectType = '1';
      }
    }
  },
  methods: {
    checkedCode1(){      //验证业务编号是否已填
      this.showCode1Tip = false;
      this.showCode2Tip = false;
      if(!this.projectData.code2){
        if(this.showCodeRequired){
          if(!this.projectData.code1){
            this.showCode1Tip = true;
          }else{
            this.showCode1Tip = false;
          }
        }
      }else{
        this.showCodeRequired = false;
      }
    },
    //业务编号和票号任填一个
    switchCode(codeType){
      ...
      // this.showCodeRequired = !this.showCodeRequired;
    },
    //选择证件类型
    setCardOption(value){
      ...
      if(this.cardTypeDictId == '01' && this.userData.cardNumber !== ''){
        ...
        .then(data=>{
          if(data.status == '200' && data.success == true){
            ...
            //alert(typeof this.projectData.applyObjectType)
            //短信通知
      ...
      timeouttimer = setTimeout(()=>{
        ...
        .then(data=>{
          if(data.status == '200' && data.success == true){
            ...
            //console.log(this.userData.businessCode)
            this.userData.falseMaterialNum = data.obj.falseMaterialNum || '';

            let _cardNumber = this.userData.cardNumber;
            if(_cardNumber.length==18){
              let _sexNum =  _cardNumber.substr(16,1);
              if(_sexNum%2!=0){
                this.userData.sex='01'
              }else{
                this.userData.sex='02'
              }
            }
          }else{
            //this.$Message.error(data.msg);
    ...
    cityChooseFun(addressName){
      if(addressName == 'XMDZ'){
          ...
          this.$refs.cityChildTXDZP.loadAreaList();
      }else{
        this.$refs.cityChildTXDZC.loadAreaList();
      }
    },
    ...
    //验证表单必填项
    submitForm(){
    	//alert(this.userData.cardType);
      return new Promise((resolve) => {
        this.$refs.projectData.validate((valid) => {
          this.$refs.userData.validate((valid2) => {
            if (valid && valid2 && this.tipName == false && this.tipNumber == false && this.tipKeymanNumber == false && this.tipMobile == false && this.tipTel == false && this.tipZipcode == false) {
              this.$Modal.confirm({
                title: '温馨提示',
    ...
    handleComplete(assignId){
      ...
      this.submitForm().then(res => {
        if(res !== 0) {
          ...
          //请求后台接口 向办事人发送短信
          if(this.usableGidsGuid.length>0){
            var result=this.usableGidsGuid;
            axios.post("/cgaCaseRegist/onlySendSMS.do",{usableGidsGuid:result.join(),mobile:this.userData.mobile})
              .then(data=>{
