<!-- 特殊情况材料   窗口登记  暂予登记 补证件登记  共用 -->

<template>
  <Row>
    <Col span="24">
      <Form ref="specialFileData" :model="specialFileData" :label-width="165">
        <Row>
          <Col span="20">
            <FormItem 
              label="特殊情况材料名称：" 
                ...
                <Col span="8">
                  ...
                  <Row v-if="uploadShow" class="uploadLocalFile">
                    <Col span="24">
                      ...
                      <form id="uploadFormSpecial" class="uploadForm">
                  ...
                  <Row v-else class="uploadAfter">
                    <Col span="24" class="uploadArea">
                      <a href="javascript:;" :title="materialName">
      ...
      <!--start  预览-->
      <ul id="dowebok36" style="display: none;">
...
export default {
  ...
  methods: {
    ...
    uploadSpecial(){ 
      let self = this;
      var formData = new FormData($("#uploadFormSpecial")[0]);
      $.ajax({  
    ...
    previews(id,name){
      ...
      if(/^.+\.png$/.test(name) == false && /^.+\.jpg$/.test(name) == false && /^.+\.bmp$/.test(name) == false){
        ...
        var viewer = new Viewer(document.getElementById('dowebok36'), {
          url: 'data-original'
        });
        setTimeout(()=>{
          $('#dowebok36 img').click();
