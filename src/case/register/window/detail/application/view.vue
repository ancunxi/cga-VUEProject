...
<template>
  <div style="margin-left:1%">
        ...
        <Col span="4" class="view">
          <a href="javascript:;" class="uploadArea" :title="item.fileName + item.fileType">
		    ...
		    <div>
				...
				<input name="请输入名称" v-focus="true" v-model='reloadName' v-if='reloadNameShowId==item.id' @blur="reNameFun(item.id)" style="margin-top:3px; margin-bottom:3px; display:block; width:80px; height:20px" />
		    </div>
    ...
    <!--start  预览-->
    <ul id="dowebok34" style="display: none;">
...
export default{
  props: ["userCardNumber","userRelatedPersonName"],
  // watch: {  
  //   userCardNumber: function(val){
  //     //console.log(val);
  //     this.userCardNumber0 = val;
  //   },
  //   userRelatedPersonName:function(val){
  //     //console.log(val);
  //     this.userRelatedPersonName0 = val;
  //   }
  // },   
  data(){
    return{
      // userCardNumber0: '',
      // userRelatedPersonName0: '',
      materialData: [],   //材料列表数据
  ...
  methods: {
    ...
    falseMaterial(viewId) {
      axios.post("/cgaCaseMaterial/examineCaseMaterialFile.do", {id:viewId, isPass:'2',cardNumber:this.userCardNumber, relatedPersonName:this.userRelatedPersonName,})
      .then(data=>{
    ...
    previews(id,type){
      ...
      if(type != '.png' && type != '.jpg' && type != '.bmp' && type != '.jpeg'){
        ...
        var viewer = new Viewer(document.getElementById('dowebok34'), {
          url: 'data-original'
        });
