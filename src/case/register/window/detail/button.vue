  ...
  <Form>
    <FormItem>
      ...
      <span v-if="current === length - 1">
        ...
        <Button type="warning" @click="handleTemporarily" style="margin-left:8px;">暂予登记</Button>
        <Button type="error" @click="handleUnpass" style="margin-left:8px;">不予登记</Button> 
        <Button type="warning" @click="handleCorrection" style="margin-left:8px;">需补正</Button>      
      </span>
          ...
          <div class="ivu-modal-btn">
            <FormItem>
              ...
              <span v-if="showNextUserSelect">
                <Button type="primary" @click="haveSelectStart" style="margin-left: 8px" :loading="haveSelectLoading">确定</Button>
              </span> 
              <span v-else>
                <Button type="primary" @click="noSelectStart" style="margin-left: 8px" :loading="noSelectLoading">确定</Button>
              </span>
            </FormItem>                
          </div>
        </Form>
...
export default{
  ...
  data(){
    return{
      haveSelectLoading:false,
      noSelectLoading:false,
  ...
  methods: {
    ...
    noSelectStart(){
      ...
      .then(data=>{
        ...
        if(data.status == '200' && data.success == true){
          ...
          this.auditModal = true;
          this.haveSelectLoading = false;
          this.noSelectLoading = false;
          return false;
        }
      }).catch(error=>{
        this.$emit('loadingHide');
    ...
    haveSelectStart(){
      this.haveSelectLoading = true;
      if(this.showNextUserSelect && this.tacheList[0].candidateList && this.tacheList[0].candidateList.length > 0){
        ...
        this.auditModal = false;
        this.$Message.error({
              content: '工作流未设置下一环节处理人，请先去设置下一环节处理人！',
              duration: 5
          });
      }
    },    
