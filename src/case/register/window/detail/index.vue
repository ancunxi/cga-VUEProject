<template>
  <div class="layout-content">
    <panel-view @sendServiceType='getServiceType'></panel-view>
    <div class="tabs">
      <Tabs type="card" v-model="tabIndex">
        ...
        <TabPane label="申请基本信息" :disabled='tabIndexA==0?false:true'>
          <tab1-view
            ...
            :usableGidsGuid="usableGidsIndex"
        ...
        <template v-if="tabFormLoad">
          ...
          <TabPane :label="tabName3">
            <rightTab2-view ref="component3"></rightTab2-view>
          </TabPane>
          <TabPane :label="tabName2">
            <rightTab3-view ref="component2"></rightTab3-view>
          </TabPane>
          <TabPane :label="tabName1">
            <rightTab4-view :cardNumber="this.$refs.tabForm1.userData.cardNumber" ref="component1"></rightTab4-view>
          </TabPane>
        </template>
      </Tabs>
    </div>
    ...
    <!--start  预览-->
    <ul id="dowebok35" style="display: none;">
...
import tab5 from './business.vue';
import tab10 from '../../../common/guide.vue';
...
export default {
  ...
  created(){
    ...
    if (typeof this.$route.query.usableGidAble != 'undefined') {
      this.usableGidsIndex = this.$route.query.usableGidAble
    }
  	this.caseIds =this.$route.query.caseId;
    //控制业务表单是否显示以及显示数量
  ...
  methods: {
    ...
    previews(id,type){
      ...
      if(type != '.png' && type != '.jpg' && type != '.bmp' && type != '.jpeg'){
        ...
        var viewer = new Viewer(document.getElementById('dowebok35'), {
          url: 'data-original'
        });
