    ...
    <div class="opinion-content">
      <Form  ref="opinionData" :model="opinionData">
        ...
        <Row class="center">
          <Col span="24">
            <FormItem>
              <Button type="primary" @click="agree">启动办理</Button>
              <Button type="error" @click="handleUnpass" style="margin-left:8px;">不予登记</Button>
              <router-link to="../index2" style="margin-left:8px;"><Button style="margin-left:8px;">返回</Button> </router-link>  
  ...
  methods: {
    ...
    handleUnpass() {
      this.$refs.opinionData.validate((valid) =>{
        if (valid) {
          this.$emit('loadingShow');
          axios.post("/cgaCase/modifyCgaCaseForRegister.do", {caseId:this.$route.query.caseId, tacheIdea:this.opinionData.tacheIdea})
          .then(data=>{
            if(data.status == '200' && data.success == true){
              this.$Message.success(data.msg);
              this.$router.push({path:'../index2',query:{moduleId:this.$route.query.moduleId}});
