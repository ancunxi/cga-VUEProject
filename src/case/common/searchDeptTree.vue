<template>
  <ul class="ivu-tree-children">
    <template v-for="(item, index) in treedata">
      <li :class="{'show-child': item.showChild||item.deptId=='1'}" @click="handleCallback($event,item)">
        <a href="javascript:;">
          <i class="ivu-icon" :class="{'ivu-icon-arrow-right-b': item.childrenList && !item.showChild && item.deptId!='1', 'ivu-icon-arrow-down-b': item.showChild||item.deptId=='1'}" @click="clickToggle($event,item)"></i>
